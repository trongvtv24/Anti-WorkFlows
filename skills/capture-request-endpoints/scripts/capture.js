  ...
  return new Promise((resolve) => {
    ...
    ws.on('message', async (raw) => {
      ...
      if (msg.method === 'Network.loadingFinished') {
        ...
        // Fetch response body asynchronously for textual MIME types only to prevent OOM
        if (fetchBody && entry.responseMimeType && isTextualMimeType(entry.responseMimeType)) {
          try {
      ...
      if (msg.method === 'Network.loadingFailed') {
        ...
        if (entry) {
          ...
          const short = entry.url.length > 80 ? entry.url.slice(0, 77) + '...' : entry.url;
          process.stdout.write(`\r[+${reqCount} req] ${entry.method} ${short} (FAILED)`.padEnd(120));
        }
      }
    });
    ...
    ws.on('close', () => {
      ...
      attachedWsUrls.delete(target.webSocketDebuggerUrl);
    });
  });
}
