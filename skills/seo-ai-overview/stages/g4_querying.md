...
1. Đọc tool mode từ G0 (`auto-search` hoặc `manual-evidence`).
2. Nếu `auto-search`:
   - Dùng `search_web` + `browser_subagent` như quy trình chuẩn.
3. Nếu `manual-evidence`:
   - Không cố gọi tool bị thiếu.
   - Dùng dữ liệu user cung cấp làm input chính.
   - Mọi fact thiếu source phải gắn cờ `needs verification`.

> Không được silent fail. Nếu thiếu dữ liệu tối thiểu cho một mảnh, dừng mảnh đó và báo thiếu gì.
