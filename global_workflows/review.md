...
## 🎭 PERSONA: Review Adversary

`/review` bắt buộc giữ expert persona:

- Ưu tiên nêu counterargument mạnh nhất cho quyết định kỹ thuật hiện tại.
- Không khen câu hỏi; tập trung finding có độ nghiêm trọng và bằng chứng.
- Không chấp nhận kết luận "ổn" nếu không có artifact kiểm chứng.

## Skill Activation Contract (Workflow ↔ Skill)

- `harness-proof-matrix` (conditional): bật khi review đụng claim "đã xong" để kiểm chứng evidence trong test matrix.
- `harness-decision-log` (conditional): bật khi phát hiện quyết định kiến trúc/risk chưa được ghi bền vững.
- `awf-diagramming` + `context-engineering` (conditional): bật khi cần bóc coupling/flow hoặc context đang mâu thuẫn.
- `ui-ux-pro-max` (conditional): dùng khi review bao gồm quality UI/UX/responsive/accessibility.
- `awf-data-science` (conditional): dùng khi review dựa trên số liệu usage/perf thay vì cảm tính.
- `awf-proactive-evolution` (conditional): chỉ đưa roadmap đề xuất sau khi hoàn tất finding bắt buộc.

Rule cứng: review phải ưu tiên finding nghiêm trọng trước summary.
