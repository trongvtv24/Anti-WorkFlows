...
HARNESS_REQUIRED_ARTIFACTS = (
    ...
    "harness/templates/spec-intake.md",
    "harness/templates/story.md",
    "harness/templates/decision.md",
    "harness/templates/validation-report.md",
    "harness/templates/high-risk-story/overview.md",
    "harness/templates/high-risk-story/design.md",
    "harness/templates/high-risk-story/execplan.md",
    "harness/templates/high-risk-story/validation.md",
)

HARNESS_TEMPLATE_SECTIONS = {
    "harness/templates/spec-intake.md": (
        "## Source",
        "## Project Summary",
...
class AwfValidator:
    ...
    def validate_harness_template_sections(self, rel_path: str, required_sections: tuple[str, ...]) -> None:
        path = self.root / rel_path
        if not path.exists():
            return
        ...
        for section in required_sections:
            if section not in text:
                self.error(rel_path, f"Missing required section: {section}")

    def validate_harness_contracts(self) -> None:
        for rel_path in HARNESS_REQUIRED_ARTIFACTS:
            path = self.root / rel_path
            if not path.exists():
                self.error(rel_path, f"Missing harness artifact: {rel_path}")
                continue
            self.counts["harness_artifacts"] += 1
